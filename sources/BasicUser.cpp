...
BasicUser::BasicUser(const std::string &username, const std::string &email, const std::string &pass,
                     ...
                     User(username, email, pass, bDay, cnp, name, phone, age),
                     car(car),
                     bal(bal)
                     {}
...
std::ostream& operator<<(std::ostream& os, const BasicUser& user) {
     os  << "========== Basic User ==========\n"
        ...
        << "\tVarsta: " << user.age << '\n'
        << "\tSuma de bani din cont: " << user.bal << '\n';
     if(!user.car)
         os << "\tUtilizatorul nu are nici o masina inchiriata\n";
     else
         os << "\tUtilizatorul are inchiriata masina: " << user.car->get_reg_plate() << '\n';
